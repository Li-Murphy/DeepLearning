...
class MyCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        return x
sample  = generate_sdf('Picture/orginal.png')
sample1 = generate_sdf('Picture/plot.png')
sample2 = generate_sdf('Picture/plot1.png')
sample3 = generate_sdf('Picture/plot2.png')
sample4 = generate_sdf('Picture/plot3.png')
sample5 = generate_sdf('Picture/plot4.png')
sample6 = generate_sdf('Picture/plot5.png')
sample7 = generate_sdf('Picture/plot6.png')
sample8 = generate_sdf('Picture/plot7.png')
...
# 打开图像文件
img = Image.open('Picture/plot.png')
# 将图像转换为numpy数组
img_array = np.array(img)
img_array = img_array.reshape([1,1108,1488,1])
...
for i in range(8):
    ...
    df = pd.DataFrame(a)
    df.to_csv(f'plot{i+1}_Base.csv', index=False, header=False)
