...
all_label = pd.read_excel('0.6ma_60_1.2.xlsx',header=None)
# 将 DataFrame 转换为 NumPy 数组
all_label = all_label.to_numpy()

np.random.seed(2)
shuffle_indices = np.random.permutation(32)
# 使用相同的打乱索引来打乱两个数组
...
validation_data = torch.Tensor(all_data[26:29].reshape([3,1,1108,1488])).to(device)
validation_label = torch.Tensor(all_label[26:29]).to(device)
test_data = torch.Tensor(all_data[29:].reshape([3,1,1108,1488])).to(device)
test_label = torch.Tensor(all_label[29:]).to(device)


all_data = torch.Tensor(all_data.reshape([32,1,1108,1488])).to(device)
all_label = torch.Tensor(all_label.reshape([32,1])).to(device)
# 定义网络结构
...
for epoch in range(epochs):
        ...
        if (epoch + 1) % 100 == 0:  # 每1000个epoch打印一次
                print(f'Epoch [{epoch + 1}/{epochs}], Loss: {loss.item()}, Error: {error*100} %')
        if max > error:
            max = error
            torch.save(model, 'NN_Base_P.pth')
# 训练完成
print('Training finished.')
model = torch.load('NN_Base_P.pth')
