...
all_data = torch.Tensor(all_data.reshape([39,1,1108,1488]))
all_label = torch.Tensor(all_label.reshape([39,1]))

model = torch.load('NN_Base_1.pth').to('cpu')
model.eval()
...
shap_values = shap_values.reshape([38,1108,1488,1])

for i in range(38):
    a = shap_values[i]
    a = a.reshape([1108,1488])
    df = pd.DataFrame(a)
    df.to_csv(f'SHAP1/plot_{i+1}_Base.csv', index=False, header=False)